...
class PGNManager(
  ...
  def loadPGN(file: File): Either[String, PGNGame] = {
    try {
      ...
      result.game match
        case Some(game) =>
          applyLoadedGame(game, result.warnings)
        case None =>
  ...
  private def buildCurrentGame(customHeaders: Map[String, String] = Map.empty): PGNGame = {
    ...
    val enrichedHeaders = PGNHeaderBuilder.withGameState(
      ...
      result = resultString,
      plyCount = gameController.moveHistoryManager.getMoveCount
    )

    val treeResult = DepthIndexedPGNBuilder.build(gameController.moveHistoryManager)
    val historyVariations = treeResult.rootVariations
    val combinedVariations = historyVariations
    val (movesWithVariations, rootVariations) = attachVariationsToMoves(treeResult.mainline, combinedVariations)
  ...
  private def applyLoadedGame(game: PGNGame, warnings: List[String]): Either[String, PGNGame] = {
    PGNGameReplayer.replay(game, gameController, board) match
      case Left(rebuildError) =>
        val warningSuffix =
          if warnings.nonEmpty then s" (warnings: ${warnings.mkString(", ")})" else ""
        Left(s"Failed to rebuild PGN position: $rebuildError$warningSuffix")
      case Right(_) =>
        // Replay variations into Move tree (primary storage)
        val variationErrors = PGNVariationReplayer.replayVariations(
          game, gameController, board, gameController.moveHistoryManager
        )
        if variationErrors.nonEmpty then
          println(s"[PGNManager] Variation replay warnings: ${variationErrors.mkString(", ")}")
