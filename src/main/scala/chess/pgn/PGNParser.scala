...
object PGNParser {
  ...
  /**
   * Parse multiple games from a single PGN string.
   * PGN files can contain multiple games separated by empty lines.
   * 
   * @param pgnText The complete PGN text potentially containing multiple games
   * @return List of parsed PGN games (empty games are filtered out)
   */
  def parseMultipleGames(pgnText: String): List[PGNGame] = {
    // Split on patterns that indicate a new game: empty line followed by headers
    // Games are separated by one or more empty lines before the next [Event ...] tag
    val gamePattern = """(?m)(?=^\[Event\s)""".r
    
    // First try to split by [Event header
    val potentialGames = gamePattern.split(pgnText).filter(_.trim.nonEmpty)
    
    if (potentialGames.length <= 1) {
      // Single game or no [Event] tags - try parsing as single game
      parsePGN(pgnText).game.toList
    } else {
      // Multiple games found
      potentialGames.flatMap { gameText =>
        val result = parsePGN(gameText)
        result.game
      }.toList
    }
  }
