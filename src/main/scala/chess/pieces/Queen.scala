...
class Queen(color: String, initialSquare: Square, board: Board) extends Piece(color, initialSquare, board, Queen) with DirectionalMovement {

  private var currentPossibleMoves: List[Square] = calculatePossibleMoves()

  override def getImage: Image = 
    val imageName = if (color == "white") "white-queen.png" else "black-queen.png"
  ...
  private def calculatePossibleMoves(): List[Square] = 
    ...
    generateDirectionalMoves(directions)
